...
function countValueChars(value: any): number {
...
function countContentBlockChars(block: any): number {
...
function countMessageChars(message: any): number {
  if (!message || typeof message !== "object") return 0;
  const content = message.content;

  if (typeof content === "string") return content.length;
  if (Array.isArray(content)) {
    return content.reduce((total: number, block: any) => total + countContentBlockChars(block), 0);
  }
