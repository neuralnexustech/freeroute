...
const DEFAULT_MITM_PORT = process.env.PORT || 20129;
const DEFAULT_MITM_ROUTER_BASE = `http://localhost:${DEFAULT_MITM_PORT}`;
...
const ENCRYPT_SALT = "freeroute-mitm-pwd";
...
async function startServer(apiKey, sudoPassword, forceKillPort443 = false) {
  ...
  if (!effectiveServerPath || !fs.existsSync(effectiveServerPath)) {
    ...
    if (!effectiveServerPath || !fs.existsSync(effectiveServerPath)) {
      throw new Error(`MITM server.js not found at ${effectiveServerPath}. Reinstall freeroute.`);
    }
  }
